...
template PaypalEmailProofOfframper(max_header_bytes, max_body_bytes, n, k, pack_size, email_extract_size, email_pack_size, levels) {
    ...
    signal offramper_email[email_extract_size] <== VarShiftLeft(max_header_bytes, email_extract_size)(in_padded, paypal_offramper_id_idx);
    signal (offramper_regex_out, offramper_regex_reveal[email_extract_size]) <== PaypalActorEmailExtraction(email_extract_size)(offramper_email);
    offramper_regex_out === 1;

    // extract the onramper id from the email
    signal onramper_email[email_extract_size] <== VarShiftLeft(max_body_bytes, email_extract_size)(in_body_padded, paypal_onramper_id_idx);
    signal (onramper_regex_out, onramper_regex_reveal[email_extract_size]) <== PaypalOnramperEmailExtraction(email_extract_size)(onramper_email);
...
component main { public [ intent_hash ] } = PaypalEmailProofOfframper(768, 6144, 121, 17, 7, 80, 20, 9);
