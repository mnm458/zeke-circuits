...
// regex: PP-Correlation-Id: [a-zA-Z0-9]+
template PaypalActorId(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		lt[1][i].in[1] <== 90;
		and[19][i] = AND();
		and[19][i].a <== lt[0][i].out;
		and[19][i].b <== lt[1][i].out;
		lt[2][i] = LessEqThan(8);
		lt[2][i].in[0] <== 97;
		lt[2][i].in[1] <== in[i];
		lt[3][i] = LessEqThan(8);
		...
		multi_or[0][i].in[0] <== and[19][i].out;
		multi_or[0][i].in[1] <== and[20][i].out;
		multi_or[0][i].in[2] <== eq[15][i].out;
		multi_or[0][i].in[3] <== eq[16][i].out;
		multi_or[0][i].in[4] <== eq[17][i].out;
		multi_or[0][i].in[5] <== eq[18][i].out;
	...
	// substrings calculated: [{(19, 20), (20, 20)}]
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(19, 20), (20, 20)]
		is_substr0[i] <== MultiOR(2)([states[i+1][19] * states[i+2][20], states[i+1][20] * states[i+2][20]]);
