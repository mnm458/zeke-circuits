...
include "./ShaBufferPoseidon.circom";

// component main = EmailVerifierPaypal(768, 7168, 121, 17, 1);
...
template EmailVerifierPaypal(max_header_bytes, max_body_bytes, n, k, ignore_body_hash_check) {
    ...
    if (ignore_body_hash_check != 1) {
        ...
        signal output email_hash_poseidon <== ShaBufferPoseidon()(sha_b64_out);

        signal pre_compute_hash;
        signal output post_compute_hash;
        (pre_compute_hash, post_compute_hash) <== MerkleDamgardVerifier(max_body_bytes)(in_body_padded, in_body_len_padded_bytes, precomputed_sha);
